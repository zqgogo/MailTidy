...
### 风险防范，尤其是防诈，钓鱼风险
比如ftx就有过多次，发送了包含链接的邮件，要求用户点击链接。模拟的跟官方很像，但是链接域名和官方渠道不完全一致。建议不要直接点击邮件链接，先从官方索赔网站或法院文件入口进入核对。
这种就必须联网搜索，确认链接是否安全，以及是否是官方渠道，相关事件等。
要建立风险等级，比如高、中、低。过高风险需要用户确认，亲自处理。
