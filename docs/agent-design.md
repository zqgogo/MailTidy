...
| **配置文件直改** | 编辑 `.mailtidy/config.json`（当前实现；全局配置后续接入） | [src/ops/config.ts](src/ops/config.ts) |
...
src/
  ...
  ops/                 ⏳ Phase 1-2（config 已接本地 JSON；logging / scheduler / audit 占位）
...
- 全局配置文件、配置写入命令、真实 pi model 选择接入非 demo recover
